...
function HomePage() {
  ...
  const { data: rankedNfts, isFetching: fetchingListedNfts } = useMoralisQuery(
      "Mythical",
      (query) => query.ascending("rank").limit(18)
  )
        ...
        <div style={styles.NFTs}>
          {NFTBalances &&
            NFTBalances.map((nft, index) => {
              const ipfsURI = nft.attributes.image;
              const imageURL = ipfsURI.replace("ipfs://", "https://ipfs.io/ipfs/")
              return (
                <Card
                  onClick={() =>
